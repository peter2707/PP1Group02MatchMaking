...
<body data-bs-spy="scroll" data-bs-target="#navbarExample" class="text-center">
        ...
        <div class="container">
            <div class="col-xl-10 offset-xl-1">
                ...
                <div class="text-start">
                    <?php
                    if ($userType == "admin") {
                        echo "<p class='card-text'><b>ID:</b> $jobmatch->id &nbsp;&nbsp;&nbsp; <b>Employer:</b> $jobmatch->employer &nbsp;&nbsp;&nbsp; <b>JobSeeker:</b> $jobmatch->jobseeker</p><h1>$jobmatch->position</h1>";
                    }elseif($userType == "jobseeker"){
                        echo "<h1>$jobmatch->position</h1>";
                    }
                    ?>
                </div>
                <?php
                        ...
                        echo <<< END
                            <div class="row d-flex align-items-center">
                                <div class="col text-start">
                                    <h5 class="text-muted"><i class='fa fa-certificate' aria-hidden='true'></i>&nbsp; $jobmatch->field</h5>
                                    <br>
                                    <small style="font-size:medium"><i class='fa fa-briefcase' aria-hidden='true'></i>&nbsp; $jobmatch->type</small><br>
                                    <small style="font-size:medium"><i class='bi bi-cash' aria-hidden='true'></i>&nbsp; $jobmatch->salary</small><br>
                                    <small style="font-size:medium"><i class='fa fa-map-marker' aria-hidden='true'></i>&nbsp; $jobmatch->location</small>
                                </div>
                                <div class="col text-end"><small><i class='fa fa-clock' aria-hidden='true'></i>&nbsp; $jobmatch->date</small></div>
                            </div>
                            <hr>
                        END;
                    } else {
                        ...
                        if ($userType == "admin") {
                            echo <<< END
                                <div class="row">
                                    <div class="col-md-2">
                                        <form method="POST">
                                            <button class="btn btn-danger-lg" onclick="return confirm('Are you sure you want to delete this match ?')" name="deny" type="submit">Delete</button>
                                        </form>
                                    </div>
    ...
    <div class="ex-basic-1 pt-4 mb-5">
        <div class="container">
                    <?php
                    if ($userType == "admin" || $userType == "jobseeker") {
                        $description = htmlspecialchars_decode($jobmatch->description);
                        $requirements = htmlspecialchars_decode($jobmatch->requirements);
                        echo <<< END
